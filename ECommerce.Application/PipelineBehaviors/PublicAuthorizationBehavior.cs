        ...
        public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
        {
            ...
            if (publicAuthRequiredCommands.Contains(request.GetType()))
            {
                ...
                if (_httpContextAccessor.HttpContext != null)
                {
                    ...
                    if (!await _userManager.IsInRoleAsync(user, ApplicationRoles.Admin) && !await _userManager.IsInRoleAsync(user, ApplicationRoles.Seller) && !await _userManager.IsInRoleAsync(user, ApplicationRoles.Buyer))
